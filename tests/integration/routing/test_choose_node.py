...
from modules.decision.adapters.outbound.weights.entropy_weights_strategy import EntropyWeightsStrategy
from modules.decision.domain.policies.decision_resolver_policy import DecisionResolverPolicy
...
async def test_choose_node_returns_best():
    ...
    result = await uc.execute(brs=BRSRequest(
        service=None,
